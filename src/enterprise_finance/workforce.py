...
FUNCTION_MIX = {
    "Software": {"R&D": 0.38, "Sales": 0.24, "Customer Success": 0.20, "G&A": 0.18},
    "Hardware": {"Sales": 0.30, "Product Management": 0.20, "Supply Chain & Commercial Ops": 0.25, "G&A": 0.25},
    "Events": {"Project Delivery": 0.45, "Sales": 0.20, "Program Management": 0.20, "G&A": 0.15},
    "Spare Parts": {"Service Operations": 0.35, "Sales": 0.20, "Planning": 0.25, "G&A": 0.20},
}

FUNCTION_COST_MULTIPLIER = {
    "R&D": 1.15, "Sales": 1.10, "Customer Success": 0.92, "G&A": 1.00,
    "Product Management": 1.12, "Supply Chain & Commercial Ops": 0.93,
    "Project Delivery": 0.90, "Program Management": 1.02,
    "Service Operations": 0.88, "Planning": 0.94,
}

DEFAULT_ENTITY_SALARY = {"DE01": 92000.0, "ES01": 62000.0, "US01": 118000.0, "JP01": 86000.0}
DEFAULT_REVENUE_PER_FTE = {"Software": 1_050_000.0, "Hardware": 1_500_000.0, "Events": 900_000.0, "Spare Parts": 1_250_000.0}
DEFAULT_NON_PEOPLE_OPEX_PCT = {"Software": 0.055, "Hardware": 0.030, "Events": 0.045, "Spare Parts": 0.030}
...
def build_workforce_schedule(operations: pd.DataFrame, config: dict, macro: pd.DataFrame | None = None) -> pd.DataFrame:
    """Build a deterministic Month x Entity x Division x Function workforce roll-forward."""
    if operations.empty:
        return pd.DataFrame()
    ...
    revenue_lookup = {(str(r.month), str(r.entity), str(r.division)): float(r.revenue) for r in monthly.itertuples(index=False)}
    macro_inflation = {}
    ...
    for month_idx, month in enumerate(months):
        for entity, division in keys.itertuples(index=False, name=None):
            entity, division = str(entity), str(division)
            ...
            effective_revenue_per_fte = float(settings["revenue_per_fte"][division]) * productivity
            trailing_revenue = float(np.mean(history[-3:])) if history else revenue
            target_total = max(float(settings["minimum_fte_per_entity_division"]), trailing_revenue * 12.0 / max(effective_revenue_per_fte, 1.0))
            ...
            for function, mix in FUNCTION_MIX[division].items():
                ...
                opening = target_fte if key not in ending_fte else ending_fte[key]
                attrition = opening * float(settings["annual_attrition"]) / 12.0
                after_attrition = max(opening - attrition, 0.0)
                hires = max(target_fte - after_attrition, 0.0) * float(settings["hire_gap_closure"])
                ending = after_attrition + hires
                average_fte = (opening + ending) / 2.0
                annual_loaded_cost = entity_salary * float(FUNCTION_COST_MULTIPLIER[function]) * float(settings["fully_loaded_factor"]) * (1.0 + wage_inflation) ** (month_idx / 12.0)
                ...
                rows.append({
                    "month": month, "entity": entity, "division": division, "function": function,
                    "opening_fte": round(opening, 4), "target_fte": round(target_fte, 4),
                    "hires": round(hires, 4), "attrition": round(attrition, 4),
                    "ending_fte": round(ending, 4), "average_fte": round(average_fte, 4),
                    "annual_loaded_cost_per_fte": round(annual_loaded_cost, 2),
                    "payroll_cost": round(payroll, 2), "recruitment_cost": round(recruitment, 2),
                    "personnel_cost": round(personnel_cost, 2), "revenue": round(revenue * float(mix), 2),
...
def enrich_operations_with_workforce(operations: pd.DataFrame, workforce: pd.DataFrame, config: dict) -> pd.DataFrame:
    if operations.empty:
        return operations.copy()
    settings = _cfg(config)
    out = operations.copy()
    totals = workforce.groupby(["month", "entity", "division"], as_index=False).agg(
        personnel_cost=("personnel_cost", "sum"), opening_fte=("opening_fte", "sum"), hires=("hires", "sum"),
        attrition=("attrition", "sum"), ending_fte=("ending_fte", "sum"), average_fte=("average_fte", "sum"),
    )
    ...
    out = out.merge(totals, on=["month", "entity", "division"], how="left").merge(revenue_total, on=["month", "entity", "division"], how="left")
    out["allocation_share"] = (out.revenue / out.entity_division_revenue.replace(0.0, np.nan)).fillna(0.0)
...
def simulate_operations_with_workforce(config: dict, months: pd.PeriodIndex, macro: pd.DataFrame) -> SimulationResult:
    ...
    return SimulationResult(operations=enriched, products=base.products, customers=base.customers, portfolio_events=base.portfolio_events)


def workforce_rollforward_checks(workforce: pd.DataFrame) -> dict:
    if workforce.empty:
        return {"workforce_rollforward_max_gap": 0.0, "workforce_negative_fte_rows": 0, "passed": False}
    ...
    return {"workforce_rollforward_max_gap": round(max_gap, 4), "workforce_negative_fte_rows": negative, "passed": max_gap <= 0.001 and negative == 0}
...
def allocation_checks(operations: pd.DataFrame, workforce: pd.DataFrame) -> dict:
    ...
    target = workforce.groupby(["month", "entity", "division"], as_index=False).personnel_cost.sum().rename(columns={"personnel_cost": "personnel_cost_schedule"})
    allocated = operations.groupby(["month", "entity", "division"], as_index=False).personnel_cost_allocated.sum()
    recon = target.merge(allocated, on=["month", "entity", "division"], how="outer").fillna(0.0)
