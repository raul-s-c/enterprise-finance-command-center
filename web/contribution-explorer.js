...
(function(root){
  ...
  const api={definitions,source,dimensions,metrics,records,aggregate};
  ...
  api.mount=data=>{
    ...
    document.getElementById('reportContext').textContent=key==='products'&&sourceName===def.enhancedSource?'Contribution explorer · entity-to-product lineage · select a bar to drill down':'Contribution explorer · independent source scope · select a bar to drill down';
    ...
    const s=settings[key]||={metric:availableMetrics[0],dimension:availableDimensions[0],month:months.at(-1)||data.meta.end_month,event:'SPEND',filters:{},page:0};
    if(!availableMetrics.includes(s.metric))s.metric=availableMetrics[0];
    if(!availableDimensions.includes(s.dimension))s.dimension=availableDimensions[0];
    ...
    function paint(){
      ...
      const reportScopeButton=document.getElementById('cx-report-scope');if(reportScopeButton)reportScopeButton.onclick=()=>{s.filters=Object.fromEntries(scoped);s.dimension=availableDimensions.find(d=>!Object.hasOwn(s.filters,d))||availableDimensions.at(-1);s.page=0;paint();document.getElementById('cx-dimension').focus();};
      document.getElementById('cx-prev').onclick=()=>{s.page--;paint();};document.getElementById('cx-next').onclick=()=>{s.page++;paint();};
      document.getElementById('cx-method').onclick=()=>reportDialog('Source & coverage',`<div class="help-pages"><p>Source: dashboard.json → ${e(sourceName)}. ${e(def.period)} ending ${e(s.month)}. Sum of ${e(s.metric)} grouped by ${e(s.dimension)}. Signed contribution = contributor / selected total; unavailable when total is zero.</p><p>${e(def.note)} Report scope can be adopted only when matching records exist. No missing balance is allocated.</p></div>`);
      host.querySelectorAll('[data-contributor]').forEach(button=>button.onclick=()=>{
        const g=visible[Number(button.dataset.contributor)],next=availableDimensions.find(d=>d!==s.dimension&&!Object.hasOwn(s.filters,d));
