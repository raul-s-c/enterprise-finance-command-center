...
(function(root){
  ...
  api.mount=data=>{
    ...
    function bridge(){
      ...
      const cumulative=[values[0],values[0]+values[1],values[0]+values[1]+values[2],values[0]+values[1]+values[2]+values[3]],all=[0,...cumulative,values[4]],max=Math.max(...all),min=Math.min(...all),span=max-min||1,y=v=>78-(v-min)/span*49;
      return `<section class="cx-bridge"><div><strong>Net working capital bridge</strong><span>PY → Actual · EUR million</span></div><svg viewBox="0 0 520 108" role="img" aria-label="Net working capital bridge from prior year to actual">${values.map((value,i)=>{const x=18+i*102,w=72;if(i===0||i===4){const top=y(value),base=y(0);return `<rect x="${x}" y="${Math.min(top,base)}" width="${w}" height="${Math.max(3,Math.abs(base-top))}" class="${i===4?'actual':'prior'}"/><text x="${x+w/2}" y="${Math.min(top,base)-5}" text-anchor="middle">${compactMoney(value)}</text><text x="${x+w/2}" y="101" text-anchor="middle" class="axis-label">${labels[i]}</text>`;}const before=cumulative[i-1],after=cumulative[i],top=Math.min(y(before),y(after)),height=Math.max(3,Math.abs(y(before)-y(after)));return `<line x1="${x-30}" y1="${y(before)}" x2="${x}" y2="${y(before)}" class="connector"/><rect x="${x}" y="${top}" width="${w}" height="${height}" class="${value>=0?'positive':'negative'}"/><text x="${x+w/2}" y="${top-5}" text-anchor="middle">${value>=0?'+':''}${compactMoney(value)}</text><text x="${x+w/2}" y="101" text-anchor="middle" class="axis-label">${labels[i]}</text>`;}).join('')}</svg></section>`;
    }
