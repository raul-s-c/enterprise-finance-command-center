...
(function(root){
  ...
  function performance(source,metric=selectedYtd,month=selectedYtdMonth){
    const rows=months(source),hasRevenue=rows.some(row=>row.revenue!==0||row.revenue_budget!==0);
    const key=metric==='ebit'||!hasRevenue?'ebit':'revenue',budgetKey=`${key}_budget`;
    ...
    return `<div class="po-ytd" aria-label="Monthly actual versus frozen Budget ${key} for the selected scope">
      <div class="po-head"><div><span>YTD ${key} · AC vs Budget</span><strong class="${delta<0?'po-adverse':'po-favorable'}">${delta<0?'−':'+'}${money(Math.abs(delta))}</strong><small>AC ${money(actual)} · Budget ${money(budget)}${hasRevenue?'':' · cost center: no revenue'}</small></div><div class="po-switch" role="group" aria-label="YTD measure"><button type="button" data-po-ytd-metric="revenue" aria-pressed="${key==='revenue'}" ${hasRevenue?'':'disabled title="No revenue in this cost-center scope"'}>Revenue</button><button type="button" data-po-ytd-metric="ebit" aria-pressed="${key==='ebit'}">EBIT</button></div></div>
  ...
  function outlook(rows,metric=selected){
    const hasRevenue=['fy_budget_revenue','fc_6_fy_revenue','fc_3_fy_revenue','fc_1_fy_revenue','latest_fy_revenue'].some(field=>sum(rows,field)!==0);
    const key=metric==='ebit'||!hasRevenue?'ebit':'revenue';
    ...
    return `<div class="po-visual" aria-label="Full-year ${key} outlook from frozen budget through published forecast vintages">
      <div class="po-head"><div><span>Latest vs frozen Budget</span><strong class="${latest-baseline<0?'po-adverse':'po-favorable'}">${latest-baseline<0?'−':'+'}${money(Math.abs(latest-baseline))}</strong><small>FY ${key} · EUR million · selected scope${hasRevenue?'':' · cost center: no revenue'}</small></div><div class="po-switch" role="group" aria-label="Outlook measure"><button type="button" data-po-metric="revenue" aria-pressed="${key==='revenue'}" ${hasRevenue?'':'disabled title="No revenue in this cost-center scope"'}>Revenue</button><button type="button" data-po-metric="ebit" aria-pressed="${key==='ebit'}">EBIT</button></div></div>
