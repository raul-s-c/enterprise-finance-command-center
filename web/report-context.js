...
(function(root){
  ...
  for(const [title,dataset] of Object.entries({'Customer aging watchlist':'ar_customer_aging','SKU aging watchlist':'inventory_sku_aging','Open customer advances':'contract_liability_detail','Customer profitability':'customer_profitability'}))register(domain(dataset,'Selected detail',[dataset]),[title]);
  ...
  register(domain('supplier-risk','Selected supplier detail',['ap_supplier_aging'],undefined,{current:true,positive:'total_ap'}),['Supplier concentration','Supplier aging watchlist']);
  ...
  register(group,['Factory absorption accounting']);
  register(group,['Base workforce plan']);
  register(domain('profitability-mix','Profitability mix',['product_family_profitability','quality_tier_profitability'],['division']),['Family economics','Quality-tier economics']);
