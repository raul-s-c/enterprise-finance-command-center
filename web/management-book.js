...
(function(root){
  ...
  function executivePages(data,state,overview){
    ...
    const overviewPage=`<article class="management-book control-tower"><header class="story-head"><div><h2>${esc(headline)}</h2><p>Revenue ${C().percent(rv.relative)} YoY · EBIT ${C().percent(ev.relative)} YoY · consolidated cash and working capital</p></div><button data-story-view="performance-review">Open monthly review</button></header>${kpis}<div class="tower-layout"><div class="tower-canvas"><div class="tower-analytics"><section class="story-region story-trend"><div class="story-region-head"><div><h3>Revenue · Actual vs prior year</h3><small>EUR million · select a month for evidence</small></div><button data-story-view="pnl">View P&amp;L</button></div>${trend}</section>${contribution(data,state)}${cashTree(data)}</div><div class="tower-bottom">${actionRail(data)}${storyLine(data)}</div></div><aside class="story-inspector" id="storyInspector" hidden><header><div><span>Selected KPI</span><h3>Evidence inspector</h3></div><button data-inspector-close aria-label="Close evidence inspector">×</button></header><div id="storyInspectorBody"></div></aside></div>${inspectorTemplates(data,state,ac,py,cf,wc)}</article>`;
    return [
      {title:'Overview',custom:true,policy:root.ReportContext.card('Revenue'),html:overviewPage},
      {title:'Drivers',custom:true,policy:root.ReportContext.card('Revenue'),html:`<div class="management-book story-board"><section class="story-region"><div class="story-region-head"><h3>Revenue to EBIT</h3><button data-story-view="pnl">Explain variance</button></div>${C().waterfall(ac,false)}</section>${contribution(data,state)}<section class="story-region story-wide">${overview.match(/<section class="report-trend">[\s\S]*?<\/section>/)?.[0]||''}</section></div>`},
