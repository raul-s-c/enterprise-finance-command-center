...
(function(root){
  ...
  function executiveMiniTrend(rows,endMonth){
    const metric=typeof reportState!=='undefined'&&reportState.metric==='ebit'?'ebit':'revenue',title=metric==='ebit'?'EBIT':'Revenue',points=M().comparisons(rows,metric,endMonth,12),scale=miniScale(points.flatMap(point=>[point.actual,point.prior]));
    return `<section class="story-region story-wide executive-mini-trend"><div class="story-region-head"><div><h3>${title} performance</h3><small>AC dark · PY gray · EUR million · select a month</small></div><div class="metric-toggle" aria-label="Chart metric"><button data-metric="revenue" aria-pressed="${metric==='revenue'}">Revenue</button><button data-metric="ebit" aria-pressed="${metric==='ebit'}">EBIT</button></div></div><div class="executive-mini-months">${points.map(point=>`<button class="executive-mini-month" data-month="${esc(point.month)}" aria-label="${esc(point.month)} ${title} actual ${money(point.actual)}, prior year ${money(point.prior)} EUR million. Show detail." title="${esc(point.month)} · AC ${money(point.actual)} · PY ${money(point.prior)} EUR m"><strong>${money(point.actual)}</strong><span class="executive-mini-bars" style="--zero:${scale.zero}%"><i class="prior" style="${scale.bar(point.prior)}"></i><i class="actual" style="${scale.bar(point.actual)}"></i></span><small>${esc(point.month.slice(2))}</small></button>`).join('')}</div></section>`;
  }
  ...
  function executivePages(data,state,overview){
    ...
    const overviewPage=`<article class="management-book control-tower">${narrative}${kpis}<div class="tower-layout"><div class="tower-canvas"><div class="tower-analytics"><section class="story-region story-trend"><div class="story-region-head"><div><h3>Revenue · Actual vs prior year</h3><small>EUR million · select a month for evidence</small></div><button data-story-view="pnl">View P&amp;L</button></div>${trend}</section>${contribution(data,state)}${cashTree(data)}</div><div class="tower-bottom">${actionRail(data)}${storyLine(data)}</div></div><aside class="story-inspector" id="storyInspector" hidden><header><div><span>Selected KPI</span><h3>Evidence inspector</h3></div><button data-inspector-close aria-label="Close evidence inspector">×</button></header><div id="storyInspectorBody"></div></aside></div>${inspectorTemplates(data,state,ac,py,cf,wc)}</article>`;
    return [
