...
(function(root){
  ...
  const responsiveChart=(chart,width,label,anchor='start',height=300)=>root.innerWidth<600&&width>root.innerWidth?`<div class="report-chart-scroll" tabindex="0" role="region" aria-label="${esc(label)} complete chart; scroll horizontally to explore" data-chart-anchor="${anchor}" style="--chart-width:${width}px;--chart-height:${height}px">${chart}</div><small class="chart-scroll-hint">Scroll chart to explore all values</small>`:chart;
  function trend(rows,metricLabel,width=800,height=320){
    ...
    const chart=svg+`<line x1="${left}" x2="${width-right}" y1="${deltaY}" y2="${deltaY}" class="axis"/></svg>`;
    // Preserve every month on phones at a readable scale, not a compressed thumbnail.
    return responsiveChart(chart,width,metricLabel,'end',height)+'<small class="chart-variance-caption">Lower bars: change vs prior year · EUR million</small>';
  }
  ...
  function series(rows,key,labelKey='month',endMonth='',width=900){
    if(root.innerWidth<600)width=Math.max(width,660);
    ...
    const legend=`<div class="chart-legend">${kinds.map(k=>`<span class="${k}-key">${{actual:'AC · Actual',plan:'PL · Plan',forecast:'FC · Forecast'}[k]}</span>`).join('')}<span>${unit} · signed zero baseline</span></div>`;
    const chart=`<svg class="report-svg series-svg" viewBox="0 0 ${width} 310" role="img" aria-label="${esc(key.replaceAll('_',' '))} · ${unit}"><defs><pattern id="${id}" width="5" height="5" patternUnits="userSpaceOnUse"><path d="M-1 1 1-1M0 5 5 0M4 6 6 4" stroke="#555" stroke-width="1"/></pattern></defs><line x1="45" x2="${width-15}" y1="${zero}" y2="${zero}" class="axis"/>${[...new Set([min,0,max])].map(v=>`<text x="40" y="${y(v)+4}" text-anchor="end" class="axis-text">${format(v)}</text>`).join('')}${rows.map((r,i)=>{
    ...
    return legend+responsiveChart(chart,width,key.replaceAll('_',' '),labelKey==='month'?'end':'start',310);
  }
  ...
  function waterfall(row,narrow=false){
    if(root.innerWidth<600)narrow=false;
    ...
    return responsiveChart(chart,900,'Revenue to EBIT bridge','start',390);
  }
