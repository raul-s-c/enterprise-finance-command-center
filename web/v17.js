...
renderers['performance-review']=function(){
  ...
  <div class="panel-grid">
    ${panel('Action lifecycle','Status composition and controlled snapshots',FinanceLifecycleVisual.status(actions,trend,table(trend,[{key:'month',label:'Close month'},{key:'active',label:'Active',num:true},{key:'overdue',label:'Overdue',num:true},{key:'carry_forward',label:'Carry-forward',num:true},{key:'closed',label:'Closed',num:true}])), 'span-5')}
    ${panel('Overdue and escalated actions','Priority increases with overdue age',FinanceLifecycleVisual.due(actions,overdue,data.meta.end_month,table(overdue,[{key:'priority',label:'Priority',format:v=>priorityBadge(v)},{key:'trigger_metric',label:'Trigger'},{key:'owner_role',label:'Owner'},{key:'due_month',label:'Due'},{key:'overdue_months',label:'Months overdue',num:true},{key:'escalation_level',label:'Escalation'}])), 'span-7')}
