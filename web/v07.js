function carryingValueRows(rows){
  const scale=Math.max(...rows.map(([,value])=>Math.abs(value)),1);
  return `<div class="carrying-value-chart" role="list" aria-label="Gross to net carrying value, EUR">${rows.map(([label,value],index)=>`<div role="listitem" class="${index===rows.length-1?'carrying-total':''}"><span>${safe(label)}</span><strong>${eur.format(value)}</strong><i><b style="width:${Math.abs(value)/scale*100}%" class="${value<0?'deduction':''}"></b></i></div>`).join('')}</div><p class="report-note">Common EUR scale. Allowances and reserves are deductions; the final row is the published net balance.</p>`;
}
