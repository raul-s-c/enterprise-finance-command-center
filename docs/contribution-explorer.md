...
The five-stage PY-to-Actual bridge uses a compact SVG coordinate system so the signed driver values and stage labels fill their available laptop panel instead of shrinking into its center. This is a presentation-only change: bridge amounts still come from the published working-capital series, with no change to the accounting engine or controls.
...
`node --test tests/*.test.cjs` includes signed contribution arithmetic, zero denominators, missing data, explicit coverage limits, CAPEX event separation, the combined NWC consolidation reserve, and aggregation reconciliation across every published period, measure and supported dimension. Tests verify source data is unchanged. Browser checks cover contribution navigation, filters, drill-down/up/reset, source-record pagination, and desktop/mobile viewport fit. Dedicated 1280 × 720 browser checks verify all eight inspectors, the last advertised evidence and contributor rows, complete flow-node labels, and the rendered NWC bridge label bounds and size.
