...
The asset-quality subpages now place source-tied visuals ahead of the original schedules. The two reserve-exposure panels share one operating selection; filters are offered only for current-month scopes with a positive inventory provision. The five largest positive customer allowances and SKU provisions are ranked on reserve amount; gross exposure and reserve rate remain separate labels. The preview also reports the sum of positive reserves in the selected source detail and the top-five share, not a fabricated group allocation. A separate, fixed-group page pairs AR and inventory aging, displaying five published gross buckets, the current risk amount, and a 12-month history of the corresponding risk share. Both bucket sums and both detail-reserve totals are tested against the published group close. Original source tables remain available from each panel. A retained operating selection never changes the consolidated aging values.
