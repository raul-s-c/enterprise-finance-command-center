...
Working capital now uses the same full-height desktop inspector so calculation, value, share, record count, source, integrity and drill path are visible together rather than concealed in an unmarked scroll area. The two-row evidence table remains directly below the contribution and signed-flow visuals. Every contribution evidence row can open its complete published record by mouse or keyboard; the table is a preview, not a claim that its truncated cells contain all source fields. `tests/ui/nwc-inspector-fit.spec.cjs` checks desktop fit, the source-record dialog, mobile keyboard access and the shared receivables row drill.

Browser Back/Forward and direct hash navigation close an open evidence or Help dialog before restoring the destination report. This prevents an old modal from obscuring a newly selected page. `tests/ui/report-dialog-history.spec.cjs` exercises both history directions while a dialog is open.
