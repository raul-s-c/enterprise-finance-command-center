...
test('Treasury shows headroom trend with the reconciled liquidity formula on a laptop',async({page})=>{
  await page.setViewportSize({width:1280,height:720});
  await page.goto('/#view=treasury&page=0');
  const cockpit=page.locator('.treasury-cockpit');
  ...
  expect(close.cash+close.undrawn_rcf-close.minimum_operating_cash-close.liquidity_headroom).toBeCloseTo(0,2);
  await expect(cockpit.locator('.sw-secondary')).toContainText('€'+(close.liquidity_headroom/1e6).toFixed(1)+'m');
  const fit=await cockpit.locator('.sw-grid').evaluate(grid=>({
    ...
    formula:grid.querySelector('.sw-secondary').getBoundingClientRect()
  }));
  expect(fit.overflow).toBeLessThanOrEqual(1);
  expect(fit.trend.right).toBeLessThanOrEqual(fit.formula.left+1);
...
test('Treasury shows all six entity balances alongside trend and formula on a tall desktop',async({page})=>{
...
test('Treasury retains formulas and KPI evidence above the 1500px breakpoint',async({page})=>{
